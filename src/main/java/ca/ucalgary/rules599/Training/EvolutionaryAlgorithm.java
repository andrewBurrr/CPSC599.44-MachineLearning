...
//                String reportOutput = absolutePath.substring(0,absolutePath.lastIndexOf(File.separator)) + "report.txt";
                Output<AccidentAttribute> output = gaImplementation.outputSortedRules(fileName, outFile, knownFile, population.individuals, configuration, configuration.getMinSupport());
        } catch (Exception e) {
            System.out.println("Error initializing stream");
        }
        return true;
